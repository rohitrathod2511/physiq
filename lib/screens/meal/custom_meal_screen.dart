...
class _CustomMealScreenState extends ConsumerState<CustomMealScreen> {
  ...
  void _logCustomMeal(CustomMeal meal) {
    ...
    final logMeal = MealModel(
      ...
      userId: user.uid,
      name: meal.name,
      calories: (meal.totalNutrition['calories'] as num?)?.toDouble() ?? 0.0,
      proteinG: (meal.totalNutrition['protein'] as num?)?.toDouble() ?? 0.0,
      carbsG: (meal.totalNutrition['carbs'] as num?)?.toDouble() ?? 0.0,
      fatG: (meal.totalNutrition['fat'] as num?)?.toDouble() ?? 0.0,
          ...
          if (!snapshot.hasData || snapshot.data!.isEmpty) {
            return Center(
              child: Column(
                ...
                children: [
                  Icon(Icons.no_food, size: 64, color: textSecondary),
          ...
          Expanded(
            child: ListView.separated(
              ...
              itemBuilder: (context, index) {
                ...
                final food = _addedFoods.firstWhere(
                  (f) => f.id == item.foodId,
                  orElse: () => _addedFoods[0],
                ); // Fallback safely
                ...
                return ListTile(
                  ...
                  subtitle: Text(
                    "${item.quantity} x ${item.unit} • $cals kcal",
                  ),
                  trailing: IconButton(
                    ...
                    onPressed: () {
                      setState(() {
                        ...
                        _addedFoods.removeWhere(
                          (f) =>
                              f.id == item.foodId &&
                              _items.where((it) => it.foodId == f.id).isEmpty,
                        );
