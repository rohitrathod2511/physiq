...
class _WaterStepsCardState extends ConsumerState<WaterStepsCard> {
  ...
  Widget build(BuildContext context) {
    ...
    final double waterOz = (widget.dailySummary['waterConsumed'] ?? 0)
        .toDouble();
    final double goalOz = (widget.dailySummary['waterGoal'] ?? 135.25)
        .toDouble();
    ...
    int currentWaterMl =
        (widget.dailySummary['waterMl'] ??
                widget.dailySummary['waterConsumed'] ??
                0)
            .toInt();
  ...
  void _showWaterEntrySheet(int current, int goal) async {
    final result = await showModalBottomSheet<Map<String, int>>(
      ...
      builder: (context) => _WaterEntrySheet(
        currentMl: current,
        goalMl: goal,
    ...
    if (result != null) {
      ...
      if (result['waterGoal'] != null && result['waterGoal'] != goal) {
        ref
            ...
            .updateWaterGoal(result['waterGoal']!);
      }
    }
  }
  ...
  Widget _buildStepCard() {
    ...
    final double stepsPercent = (_stepsGoal > 0)
        ? (steps / _stepsGoal).clamp(0.0, 1.0)
        : 0.0;
        ...
        children: [
          Container(
            ...
            decoration: BoxDecoration(
              ...
              borderRadius: BorderRadius.circular(20),
              border: Border.all(
                color: isSelected ? AppColors.water : Colors.grey.shade200,
              ),
            ),
            child: Icon(
              icon,
              color: isSelected ? Colors.white : AppColors.water,
              size: 30,
            ),
          ),
          const SizedBox(height: 8),
